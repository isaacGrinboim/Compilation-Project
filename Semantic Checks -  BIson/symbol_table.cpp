...
bool TableStack::check_override(string id, string return_type, vector<string> param_types){
...
Symbol *TableStack::checkTheRightSymbol(const string &name, vector<string> type_of_params, int yylineno) {
    int j = 0;
    ...
    for (auto it = table_stack.begin(); it != table_stack.end(); ++it) {
        if ((*it)->symbols.size()) {
            ...
            for (auto it1 = (*it)->symbols.begin(); it1 != (*it)->symbols.end(); ++it1) {
                if ((*it1)->name == name && type_of_params.size() == (*it1)->params.size()) {
    ...
    if (j == 0) {
        ...
        output::errorPrototypeMismatch(yylineno, name, type);
        exit(0);
    }
    ...
    for (auto it = scope->symbols.begin(); it != scope->symbols.end(); ++it) {
        ...
        if ((*it)->is_function) {
            ...
            output::printID((*it)->name, 0,
                            output::makeFunctionType(converted_params[0], convert_to_upper_case((*it)->type)));
        } else {
